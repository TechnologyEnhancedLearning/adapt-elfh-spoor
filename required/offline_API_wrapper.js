...
var GenericAPI = {
  ...
  store: function(force) {
    ...
    if (!force && window.Cookies.get('_elfh_spoor') === undefined) return;

    window.Cookies.set('_elfh_spoor', this.data);

    // a length mismatch will most likely indicate cookie storage limit exceeded
    if (window.Cookies.get('_elfh_spoor').length !== JSON.stringify(this.data).length) {
  ...
  fetch: function() {
    ...
    this.data = window.Cookies.getJSON('_elfh_spoor');
  ...
  reset: function() {
    window.Cookies.remove('_elfh_spoor');
  },
